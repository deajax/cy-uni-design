<template>
	<view class="cy-goods-item--wrapper" :class="bordered ? '' : 'cy-goods-item--borderless'">
		<view
			:class="['cy-goods-item', `cy-goods-item--${layout}`, round ? 'cy-goods-item--round' : '', setClass, customClass]"
		...
		>
			...
			<view class="cy-goods-item__image">
                <view class="cy-goods-item__sellout" v-if="sellout"></view>
				<slot name="image" />
				<image class="cy-goods-item__image-image" :src="image" :mode="imageMode" />
			</view>
			<view :class="['cy-goods-item__content', customClassContent]">
				<view :class="['cy-goods-item__content--title', customClassTitle]">
					<text class="cy-goods-item__content--title-text" v-if="titlePrefix">{{ titlePrefix }}</text>
					<text class="cy-goods-item__content--title-text" :class="ellipsis ? 'cy-ellipsis-1' : 'cy-ellipsis-2'">{{ title }}</text>
					<view
						v-if="description"
						:class="['cy-goods-item__content--description', customClassDescription]"
					>
						{{ description }}
					</view>
					<slot name="title" />
				...
				<view :class="['cy-goods-item__content--note', customClassNote]">
					<view>
						<view :class="['cy-goods-item__origin-price', customClassOriginPrice]">
							<template v-if="originPrice">
								¥{{ originPrice }}
							</template>
						...
						<view :class="['cy-goods-item__price', customClassPrice]">
							...
							/>
							<text :class="['cy-goods-item__price--unit', customClassPriceUnit]" v-if="unit">{{ unit }}</text>
							<slot name="price" />
						</view>
					</view>
    ...
    methods: {
        ...
        isEmpty(data) {
            ...
            if (data === "" || data === "" || data === "undefined" || data === undefined || data === null) {
                return true;
            } else if (JSON.stringify(data) === "{}" || JSON.stringify(data) === "[]" || JSON.stringify(data) === "[{}]") {
                return true;
            }
            return false;
        },
