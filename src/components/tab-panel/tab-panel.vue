<template>
	<view class="cy-tab__panel" :data-index="index" :class="[uniquePaneClass, paneClass, customClass]" :style="[paneStyle]">
		<!-- 渲染过的则不再渲染，未渲染的根据激活状态进行渲染 -->
		<view class="cy-tab__panel--wrap" v-if="rendered ? true : active">
			<slot />
		</view>
	</view>
</template>
...
<script>
import { isNull, toClass, getUid } from '../tabs/uitls';
...
export default {
	...
	computed: {
		...
		// 保证唯一的样式
		uniquePaneClass() {
			return 'cy-tab__panel' + this.unqieKey
		},
		// 内容class
		paneClass() {
	...
	methods: {
		...
		async observePane(top) {
			...
			// 注意：如果cy-tabs使用的区域滚动，整个页面的布局跟随页面滚动，当pane跟随页面移动了之后，
			//      那么cy-tabs__content的top就会变化，导致交互区域位置不准确，可以在onPageScroll使用定时器实现滚动结束的处理重新resize一下组件创建pane的监听
			...
			// 不能观察根节点 unk-vendors.js:14596 [system] Node .cy-tab__panel9 is not found. Intersection observer will not trigger.
			paneObserver.observe(`.${this.uniquePaneClass} .cy-tab__panel--wrap`, res => {
