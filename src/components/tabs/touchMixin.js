	...
	return {
		...
		methods: {
			...
			touchMove(event) {
				...
				this.tonchOpt.deltaX =
					touch.clientX < 0
						? 0
						: this.tonchOpt.startX - touch.clientX;
				...
				if (this.tonchOpt.direction === "horizontal") {
					...
					if (
						(isRight && currentIndex === 0) ||
						(!isRight && currentIndex === dataLen - 1)
					) {
			...
			touchEnd() {
				if (this.tonchOpt.moved) {
					...
					const isChange =
						speed > 0.25 || distance >= Number(this.swipeThreshold); //是否切换
					const currIndex = this.currentIndex; //当前选中下标
					const targetIndex = isChange
						? this.tonchOpt.nextIndex
						: currIndex; //目标标签的下标
					this.touchEndForPane(
						this.tonchOpt.deltaX,
						currIndex,
						targetIndex,
						isChange
