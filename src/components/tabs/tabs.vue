<template>
	<view class="cy-tabs" :class="[uniqueTabsClass, tabsClass, customClass]">
		<view class="cy-tabs__depend--offset" :style="[dependOffsetStyle]" />
		<!-- 依赖元素，用于处理滚动吸顶所需 -->
		<view class="cy-tabs__depend" :style="[dependStyle]" />
		<!-- 依赖元素,用于模拟透明标签栏渐变所需 -->
		<view v-if="transparent" class="cy-tabs__depend--transparent" :style="[dependTranStyle]" />

		<!-- 标签栏区域 -->
		<!-- touchmove.prevent阻止触摸移动事件 -->
		<view class="cy-tabs__wrap" :style="[innerWrapStyle, wrapStyle]" :class="[wrapClass, customClassWrapper]">
			<!-- 标签栏左侧插槽 -->
			<view :class="['cy-tabs__nav-left', customClassLeft]">
				<slot name="nav-left" />
			</view>

			<!-- 标签栏 -->
			<scroll-view :class="['cy-tabs__scroll', customClassScroll]"
						 :scroll-x="scrollX"
						 :scroll-y="scrollY"
						 ...
						 @touchmove.stop="() => { }">
				<view class="cy-tabs__nav" :class="[navClass, customClassNav]" :style="[navStyle]">
					<!-- 标签项 -->
					<view class="cy-tab"
						  v-for="(tab, index) in tabs"
						  :key="tab.key"
						  @tap.stop="onClick(index)"
						  :class="[tabClass(index, tab), tab.titleClass, customClassTab]">
						<!-- 标题 -->
						<view :class="['cy-tab__title', titleClass(tab), customClassTitle]">
							<!-- 图标或图片 -->
							<view class="cy-tab__icons" v-if="tab.iconType || tab.imageSrc">
								<cy-icon v-if="tab.icon" :name="tab.icon.name || tab.icon"
										 :size="Number(tab.icon.size) || 36" />
								<image class="cy-tab__image" v-if="tab.imageSrc" :mode="tab.imageMode"
									   :src="tab.imageSrc" />
							</view>
							<cy-badge v-if="tab.dot || tab.badge" :dot="tab.dot" :count="tab.badge"
									  :content="tab.title" />
							<!-- 标签文本 -->
							<view v-else class="cy-tab__text" :class="[textClass(tab)]">
								<!-- 
									统一改为具名的作用域插槽（暂时不做）:
										1.避开vue3中小程序端循环生成动态插槽名的插槽时后备内容不显示
										2.在小程序使用template会被编译成view,导致cy-tab__text样式无效,标签栏未超过滚动数量时超出隐藏也同样失效,H5和APP不会
										...
										4.微信报： 
										    ...
										    b.[Component] More than one slot named "title" are found inside a single component instance (in component "uni_modules/cy-tabs/components/cy-tabs/cy-tabs"). The first one was accepted.
								-->
					...
					<view v-if="isSlideBlock" class="cy-tabs__bar" :class="[barClass, customClassBar]"
			...
			<!-- 标签栏右侧插槽 -->
			<view :class="['cy-tabs__nav-right', customClassRight]">
				<slot name="nav-right" />
			</view>
		</view>

		<!-- 标签栏占位容器 -->
		<view class="cy-tabs__placeholder" :style="[placeholderStyle]" />

		<!-- 标签内容 -->
		<view class="cy-tabs__content" :class="[contentClass, customClassContent]" @touchstart="touchStart"
			  @touchmove="touchMove"
			  @touchend="touchEnd">
			<view class="cy-tabs__track" :class="[{ 'is-scrollspy': scrollspy }]" :style="[trackStyle]">
				<!-- 滚动导航与侧边栏导航的内容区域：使用scroll-view实现区域滚动，否则就是页面级滚动 -->
				<scroll-view v-if="scrollspy && !pageScroll" class="cy-tabs__content-scrollview" scroll-y
...
export default {
	name: 'cy-tabs',
	externalClasses: ['custom-class', 'custom-class-wrapper', 'custom-class-left', 'custom-class-right', 'custom-class-scroll', 'custom-class-nav', 'custom-class-tab', 'custom-class-title', 'custom-class-bar', 'custom-class-content'],
	options: {
	...
	emits: [
		...
		'slide-change', //内容页滑动时触发（仅barAnimateMode为ease、worm、worm-ease时有效） 回调参数：{ dx：滑动距离； rate：当前滑动长度占滑动区域的比例；targetIndex：目标下标；}
		'slide-end' //内容页滑动结束时触发（仅barAnimateMode为ease、worm、worm-ease时有效） 回调参数：{ targetIndex：目标下标；}
	],
	props: {
		...
		barAnimateMode: {
			...
			default: "ease",
			validator(value) {
				return ['none', 'ease', 'worm', 'worm-ease'].includes(value);
			}
		},
		// 标签宽高是否动态变化
		...
		// 是否开启标签内容滑动时的拖动动画
		// swipeable为true时有效，建议设置is-lazcy-render=false。（该属性开启时考虑给包裹内容的容器增加一个min-height，因为其他未显示的标签内容会沿用当前显示的高度，拖动切换后由于高度不一致会有回弹）
		swipeAnimated: {
			type: Boolean,
			default: true,
		},
	...
	computed: {
		...
		// 滑块切换模式为线性运动
		isLineMode() {
			return this.isLine && this.barAnimateMode === "ease"
		},
		// 滑块切换模式为毛毛虫运动
		isWormMode() {
		...
		// 滑动动画时长(ms)
		msDuration() {
			return this.animated ? Number(this.duration) : 0;
		},
		// 在设置滚动条位置时使用动画过渡
		...
		barAnimatedStyle() {
			...
			const style = {
				...
				transition: this.barAnimated ? `all ${this.duration}ms ease` : 'none',
			}
		...
		// line风格的底部条宽度自适应选中标签宽度，仅对barAnimateMode为ease生效
		autoLineBarWidth() {
			return this.isLine && this.barAnimateMode === "ease" && this.barWidth === "auto"
		},
		// line风格的底部条高度自适应选中标签高度，仅对barAnimateMode为ease生效
		autoLineBarHeight() {
			return this.isLine && this.barAnimateMode === "ease" && this.barHeight === "auto"
		},
		// 滑块内容样式
		barInnerStyle() {
			const style = {};
			// 水平和垂直的处理
			style.transition = this.barAnimated ? `all ${this.duration}ms ease` : 'none';
	...
	watch: {
		...
		isFixed(val) {
			this.$emit('stickcy-change', { isFixed: val }); // emit 吸顶变化事件
	...
	methods: {
		...
		tabClass(index, tab) {
			return toClass({
				...
				'cy-ellipsis': this.isEllipsis,
				'cy-tab__prev': index === this.currentIndex - 1, //与当前标签相邻的前面的标签
				'cy-tab__next': index === this.currentIndex + 1 //与当前标签相邻的后面的标签
			},
				`is-${this.type}`,
				`cy-tab_${index}`
			);
		...
		// 标签区域class
		titleClass(tab) {
			return toClass({ '': this.isEllipsis }, `cy-tab__title--${tab.position}`);
		},
		// 标签文字class
		textClass(tab) {
			return toClass({ 'cy-ellipsis': this.isEllipsis }, `cy-tab__text--${tab.position}`);
		},
		// 初始化操作
		async init(callback) {
			try {
				this.wrapRect = await this.getRect('.cy-tabs__wrap'); //标签栏包裹层节点信息（在小程序中，偶发性高度获取不正确）
				this.scrollViewRect = await this.getRect('.cy-tabs__scroll'); //标签栏scroll-view的节点信息
				this.barRect = await this.getRect('.cy-tabs__bar'); //滑块节点信息
				this.contentRect = await this.getRect('.cy-tabs__content'); //标签内容节点信息
				this.dependOffsetRect = await this.getRect('.cy-tabs__depend--offset'); //标签栏吸顶设置offset时的模拟元素
				// nav-left、nav-right插槽宽度、高度,计算额外宽高
				const r1 = await this.getRect('.cy-tabs__nav-left');
				const r2 = await this.getRect('.cy-tabs__nav-right');
		...
		async observeSticky() {
			...
			// 绑定观察的元素
			dependObserver.observe(`.${this.uniqueTabsClass} .cy-tabs__depend`, res => {
				// 判断是否达到吸顶条件范围
		...
		async observerTransparent() {
			...
			const rect = await this.getRect('.cy-tabs__depend--transparent');
			...
			tranObserver.observe('.cy-tabs__depend--transparent', res => {
		...
		async scrollToCurrentContent(immediate = false) {
			...
			if (this.pageScroll) {
				...
				const rect = await this.getRect('.cy-tabs__content'); //实时获取top
				top -= (rect?.top || 0);
			}
		...
		changeTrackStyle(isSlide = false, duration, offsetWidth = 0) {
			...
			// this.trackStyle = { left, transition：this.animated ? `left ${duration}ms ease` : 'none' };

			// fix（v2.2.8）：
			// 在微信开发者工具上,使用了left，transition有时会失效，left效果不好，仍使用transform
			// 需避免有fixed的元素嵌套在cy-tab中
			this.trackStyle = {
				transform: `translate(${left}, 0px) translateZ(0px)`,
				transition: this.animated ? `transform ${duration}ms ease` : 'none',
			}
		},
		// 改变标签内容样式
		async changePaneStyle() {
			...
			if (!curr) return;
			curr.paneStyle = null;
			const rect = await curr.getRect('.cy-tab__panel--wrap');
		...
		getTabScrollOffset() {
			return new Promise((resolve, reject) => {
				...
				const nodeRef = this.getSelectorQuery().select(`.${this.uniqueTabsClass} .cy-tabs__scroll`);
		...
		getScrollViewRect() {
			return new Promise((resolve, reject) => {
				...
				const nodeRef = this.pageScroll ? query.selectViewport() : query.select(
					`.${this.uniqueTabsClass} .cy-tabs__content-scrollview`);
		...
		updateTabsWrapRect() {
			...
			this.$nextTick(async () => {
				this.wrapRect = await this.getRect('.cy-tabs__wrap'); //标签栏节点信息
				this.scrollViewRect = await this.getRect('.cy-tabs__scroll'); //标签栏scroll-view的节点信息
		...
		async updateTabRect(isFull = false) {
			...
			if (isFull) {
				...
				// 获取tab的节点信息
				const rects = await this.getBatchRect(this.tabs.map((_, i) => `.cy-tab_${i}`));
				...
				const rects = await this.getBatchRect(idxs.map(i => `.cy-tab_${i}`));
		...
		handleWormAnimated(deltaX, currIndex, targetIndex, isChange) {
			...
			return new Promise(resolve => {
				...
				this.intervalFn((clear) => {
					if (dxList.length === 0) {
						//内容页滑动结束时触发（仅barAnimateMode为ease、worm、worm-ease时有效）
						...
						if (values) {
							...
							// 内容页滑动时触发（仅barAnimateMode为ease、worm、worm-ease时有效）
		...
		//在swiper的@animationfinish中通知cy-tabs对setDx的锁定，若在父组件中调用了setDx，则必须调用unlockDx
		unlockDx() {
			this.$nextTick(() => {
				this.barAnimated = true;
				this.shouldSetDx = true;
